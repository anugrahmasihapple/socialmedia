    ...
    <div className='App'>
      ...
      <div className='form'>
        <form onSubmit={changeOnClick} encType='multipart/form-data'>
          ...
          />
          <br />
          <button className='btn' type='submit' onChange={onChangeFile}>
            Add to List
          </button>
        </form>
      </div>
      ...
      <div>
        ...
        {foodList.map((val, key) => {
          const base64String = btoa(
            String.fromCharCode(
              ...new Uint8Array(val?.articleImage?.data?.data)
            )
          );

          return (
            <div className='food' key={key}>
              <h1 id="name">{val.foodName}</h1>
              <h1 id="names">{val.daysSinceIAte}</h1>

              <div>
                 <img src={user} alt="user logo"  id="user" />
               </div>
